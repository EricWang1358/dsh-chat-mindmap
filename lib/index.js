...
function optionalService(ctx, name) { return ctx.reflect?.get(name, true); }
function startPanelRegeneration(ctx, libraryId, input, panelRuns, activeByLibrary) {
    if (activeByLibrary.has(libraryId))
        throw new InputError('该脑图正在重新生成，请等待或取消当前任务', 409);
    const agents = optionalService(ctx, 'agents');
    const runtime = optionalService(ctx, 'subagents');
        ...
        handler: async (req, res) => {
            try {
                ...
                if (req.method === 'GET' && url.pathname.endsWith('/health')) {
                    const agents = optionalService(ctx, 'agents');
                    const subagents = optionalService(ctx, 'subagents');
                ...
                if (panelRunMatch && req.method === 'GET') {
                    ...
                    writeJson(res, 200, { ok: true, value: { runId: run.runId, libraryId: run.libraryId, status: run.status, detail: run.detail, ...(run.revisionId ? { revisionId: run.revisionId } : {}) } });
                    return;
                }
