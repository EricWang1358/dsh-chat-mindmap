...
import { buildMindmap, buildMindmapFromOutline, countMindmapNodes, mindmapNodeNotesForPrompt, mindmapToMarkdown, validateMindmapDocument } from './core.js';
...
function parseString(value, field, maxLength, required = false) {
    ...
    if (typeof value !== 'string' || value.length > maxLength)
        throw new InputError(`${field} must be a string of at most ${maxLength} characters`);
...
function parseSource(value) {
    if (typeof value === 'undefined')
        return undefined;
    if (!isRecord(value))
        throw new InputError('source must be an object');
    const kinds = new Set(['text', 'pdf', 'image', 'document', 'chat', 'unknown']);
    const kind = parseString(value.kind, 'source.kind', 32, true);
    if (!kinds.has(kind))
        throw new InputError('source.kind is invalid');
    ...
    if (typeof value.metadata !== 'undefined') {
        if (!isRecord(value.metadata))
            throw new InputError('source.metadata must be an object');
        const metadata = {};
...
function parseConfig(value) {
    ...
    const config = {};
    for (const key of ['layout', 'theme', 'font', 'instruction', 'language']) {
        const parsed = parseString(value[key], `config.${key}`, key === 'instruction' ? 4_000 : 100);
        if (parsed !== undefined)
            config[key] = parsed;
    }
    if (typeof value.density !== 'undefined') {
        if (value.density !== 'compact' && value.density !== 'standard' && value.density !== 'detailed')
            throw new InputError('config.density is invalid');
        config.density = value.density;
    }
    for (const key of ['maxNodes', 'contextLimit']) {
        if (typeof value[key] !== 'undefined') {
            if (typeof value[key] !== 'number' || !Number.isFinite(value[key]))
                throw new InputError(`config.${key} must be a number`);
            config[key] = value[key];
        }
    }
    return config;
}
function parseGenerateInput(value) {
    if (!isRecord(value))
...
function parseCreateMapInput(value) {
    if (!isRecord(value))
        throw new InputError('request body must be an object');
    let document;
    try {
...
function parsePatchInput(value) {
    ...
    if (typeof patch.title === 'undefined' && typeof patch.document === 'undefined' && typeof patch.config === 'undefined' && typeof patch.archived === 'undefined')
        throw new InputError('patch must include an editable field');
    return patch;
}
function parseRegenerateInput(value) {
    if (!isRecord(value))
        throw new InputError('request body must be an object');
...
function parsePresentInput(value) {
    if (!isRecord(value))
        throw new InputError('request body must be an object');
    return {
...
function requestSecurityError(req) {
    ...
    if (site === 'cross-site' || site === 'none')
        return new InputError('cross-site request rejected', 403);
    const origin = headers.origin;
    if (origin) {
        if (origin === 'null')
            return new InputError('opaque origin rejected', 403);
        let parsed;
        try {
            parsed = new URL(origin);
        }
        catch {
            return new InputError('invalid request origin', 403);
        }
        const host = headers.host;
        if (!host || parsed.host !== host || !['http:', 'https:'].includes(parsed.protocol))
            return new InputError('origin is not the DSH web origin', 403);
    }
    if (!['GET', 'HEAD', 'OPTIONS'].includes(req.method ?? '')) {
        if (headers['x-dsh-chat-mindmap-request'] !== '1')
            return new InputError('plugin request header required', 403);
    }
    const remote = req.socket?.remoteAddress;
    if (remote && !['127.0.0.1', '::1', '::ffff:127.0.0.1'].includes(remote))
        return new InputError('non-loopback request rejected', 403);
    return null;
}
...
function decodeRevisionId(encoded) {
    const revisionId = decodeId(encoded);
    if (!/^rev-[a-f0-9]{24}$/.test(revisionId))
        throw new InputError('invalid revision id');
    return revisionId;
}
async function generateResult(args, sessionId) {
    const options = {
    ...
    const source = args.source ?? { kind: 'unknown', ...(sessionId ? { sessionId } : {}) };
...
function asOutline(value) {
    if (!isRecord(value) || typeof value.title !== 'string' || typeof value.outline !== 'string' || !value.title.trim() || !value.outline.trim() || value.title.length > 120 || value.outline.length > 200_000)
        throw new Error('子代理没有返回有效的脑图大纲');
    return { title: value.title.trim(), outline: value.outline.trim() };
}
function regenerationPrompt(record, instruction) {
    if (!record)
...
function startPanelRegeneration(services, libraryId, input, panelRuns, activeByLibrary) {
    if (activeByLibrary.has(libraryId))
        throw new InputError('该脑图正在重新生成，请等待或取消当前任务', 409);
    const parent = services?.agents.get(SessionId(input.sessionId));
    const runtime = services?.subagents;
    if (!parent || !runtime?.getProvider('fork'))
        throw new InputError('当前会话不支持 fork 子代理重新生成', 503);
    const runId = `panel-${Date.now().toString(36)}-${randomUUID().replaceAll('-', '').slice(0, 8)}`;
    const controller = new AbortController();
    const panelRun = { runId, libraryId, status: 'running', detail: '正在由 fork 子代理整理脑图大纲…', controller };
    ...
    void (async () => {
        try {
            ...
            if (controller.signal.aborted) {
                panelRun.status = 'cancelled';
                panelRun.detail = '已取消重新生成';
                return;
            }
            if (result.stopReason !== 'completed')
                throw new Error(result.diagnostic || '子代理未完成重新生成');
            ...
            panelRun.status = 'completed';
            panelRun.detail = `重新生成完成：${countMindmapNodes(saved.current.root)} 个节点`;
            panelRun.revisionId = revisionIdOf(saved.current);
        }
        catch (error) {
...
function windowOrGlobalTimeout(callback, milliseconds) { return setTimeout(callback, milliseconds); }
async function presentResult(args) {
    const record = await getMindmap(args.libraryId);
    if (!record)
        return { libraryId: args.libraryId, revisionId: args.revisionId, title: 'Mind map', nodeCount: 0, state: 'expired', capabilityNote: CAPABILITY_NOTE };
    ...
    if (!document)
        return { libraryId: args.libraryId, revisionId: args.revisionId, title: record.title, nodeCount: 0, state: 'expired', capabilityNote: CAPABILITY_NOTE };
...
function presentContent(value) {
    ...
    return [{ type: 'text', text: `dsh-chat-mindmap-preview:${JSON.stringify({ libraryId: value.libraryId ?? '', revisionId: value.revisionId ?? '', title: value.title ?? 'Mind map', nodeCount: value.nodeCount ?? 0, state: value.state ?? 'expired', capabilityNote: value.capabilityNote ?? CAPABILITY_NOTE })}` }];
}
export function apply(ctx) {
    ...
    injectOptional?.(['agents', 'subagents'], (serviceCtx) => {
        const services = { agents: serviceCtx.agents, subagents: serviceCtx.subagents };
        panelServices = services;
        serviceCtx.effect(() => () => { if (panelServices === services)
            panelServices = undefined; }, 'chat-mindmap: panel fork capability');
    });
    const generate = defineTool({
        ...
        output: {
            ...
            // Keep both durable identifiers in the model-visible result. The next
            // present_chat_mindmap call must never infer an ID from title/text.
            render: (_args, value) => [{ type: 'text', text: `Generated mind map “${value.title}” with ${value.nodeCount} nodes. libraryId=${value.libraryId} revisionId=${value.revisionId}\nCall present_chat_mindmap with libraryId and revisionId to show the SVG preview.\n\n${value.markdown}` }],
        },
        timeoutMs: 30_000,
    ...
    const unregisterApi = (path) => ctx.effect(() => ctx.webServer.register({
        kind: 'prefix',
        path,
        handler: async (req, res) => {
            const securityError = requestSecurityError(req);
            if (securityError) {
                writeJson(res, securityError.status, { ok: false, error: securityError.message });
                return;
            }
            try {
                ...
                if (panelRunMatch && req.method === 'GET') {
                    ...
                    if (!run) {
                        writeJson(res, 404, { ok: false, error: 'panel run not found' });
                        return;
                    }
                    writeJson(res, 200, { ok: true, value: { runId: run.runId, libraryId: run.libraryId, status: run.status, detail: run.detail, ...(run.noteLength ? { noteLength: run.noteLength } : {}), ...(run.childId ? { childId: run.childId } : {}), ...(run.revisionId ? { revisionId: run.revisionId } : {}) } });
                    return;
                }
                ...
                if (req.method === 'GET' && url.pathname.endsWith('/maps')) {
                    ...
                    if (archivedParam && archived === undefined)
                ...
                if (req.method === 'GET' && revisionMatch) {
                    ...
                    const record = await getMindmap(libraryId);
                    if (!record) {
                        writeJson(res, 410, { ok: false, error: 'mindmap revision expired' });
                        return;
                    }
