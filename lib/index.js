        ...
        handler: async (req, res) => {
            try {
                ...
                if (panelRunMatch && req.method === 'GET') {
                    ...
                    writeJson(res, 200, { ok: true, value: { runId: run.runId, libraryId: run.libraryId, status: run.status, detail: run.detail, ...(run.childId ? { childId: run.childId } : {}), ...(run.revisionId ? { revisionId: run.revisionId } : {}) } });
                    return;
                }
