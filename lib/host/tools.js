...
function workspaceMismatch(record, callerWorkspace) {
...
function executeLaunch(deps, rawArgs, exec) {
    ...
    return (async () => {
        ...
        if (workspaceMismatch(existing, workspaceKey))
            throw new DomainError('MINDMAP_NOT_FOUND', 'mindmap not found');
        if (!existing && !workspaceKey)
            throw new DomainError('CAPABILITY_UNAVAILABLE', 'workspace identity unavailable');
