...
function textFromSubagentOutput(output) {
    ...
    return output
        ...
        if (typeof block === 'string')
            return block;
        if (typeof block === 'object' && block !== null && 'type' in block && block.type === 'text' && typeof block.text === 'string')
            return block.text;
        return '';
    })
        .join('')
...
function parseTextOutline(output) {
...
function unwrapStructured(value, depth = 0) {
    if (depth > 3 || value === undefined || value === null)
        return undefined;
    if (typeof value === 'string' || Array.isArray(value))
        return parseTextOutline(value);
    if (typeof value !== 'object')
        return undefined;
    const record = value;
    if (typeof record.title === 'string' && typeof record.outline === 'string')
        return record;
    if (typeof record.title === 'string' && typeof record.content === 'string')
        return { title: record.title, outline: record.content };
...
function outlineCandidates(result) {
    const candidates = [];
...
function validatedOutlineOf(result) {
    let lastError;
    for (const candidate of outlineCandidates(result)) {
        try {
            return validateAgentOutlineResult(candidate);
        }
        catch (error) {
            lastError = error;
        }
...
export async function runOutlineGeneration(services, input, opts = {}) {
    ...
    try {
        ...
        if (outlineCandidates(result).length === 0 && result.stopReason !== 'completed')
