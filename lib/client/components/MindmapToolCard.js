...
export function registerSnapshotFetcher(fetcher) {
    snapshotFetcher = fetcher;
    return () => {
        if (snapshotFetcher === fetcher)
...
async function loadMindMap() {
    const module = await import('../mindmap.js');
    return module.default;
}
function toSimpleMindMapData(node) {
    return {
...
async function renderSvgBlob(document, config) {
...
export function CardBody(props) {
    const state = cardStateOf(props.reference, props.url, props.error);
    ...
    return (_jsxs("section", { style: { padding: '10px', border: '1px solid var(--dsw-alias-border-l2,#475569)', borderRadius: '8px', maxWidth: '620px' }, children: [_jsx("strong", { children: reference.title }), _jsxs("small", { style: { display: 'block', opacity: 0.7, marginBottom: '8px' }, children: [reference.nodeCount, " \u8282\u70B9 \u00B7 SVG \u9884\u89C8"] }), reference.capabilityNote ? _jsx("small", { style: { display: 'block', opacity: 0.62, marginBottom: '8px' }, role: 'note', children: reference.capabilityNote }) : null, state.kind === 'failed' ? _jsx("span", { role: 'status', children: state.note }) : null, state.kind === 'loading' ? _jsx("span", { role: 'status', children: "\u6B63\u5728\u751F\u6210 SVG \u9884\u89C8\u2026" }) : null, state.kind === 'ready' && props.url ? (_jsx("button", { type: 'button', style: { display: 'block', padding: 0, border: 0, background: 'transparent', cursor: 'zoom-in' }, "aria-label": '打开 ' + reference.title + ' SVG 预览', onClick: props.onOpen, children: _jsx("img", { src: props.url, alt: reference.title + ' 思维导图', style: { display: 'block', maxWidth: '100%', maxHeight: '360px', background: 'var(--dsw-alias-bg-base,#fff)', borderRadius: '6px' } }) })) : null] }));
}
export function MindmapToolCard({ block }) {
    ...
    return (_jsxs(_Fragment, { children: [_jsx(CardBody, { reference: reference, url: url, error: error, onOpen: () => setOpen(true) }), open && url && reference && reference.state !== 'expired' ? (_jsx(SvgPreviewDialog, { src: url, alt: reference.title + ' 思维导图', onClose: () => setOpen(false) })) : null] }));
}
