...
export class OnboardingPreference {
    ...
    optimisticSeen;
    get seen() {
        if (this.optimisticSeen !== undefined)
            return this.optimisticSeen;
        const snapshot = this.scope?.getSnapshot();
        if (snapshot?.status === 'ready' && snapshot.value)
            return snapshot.value.onboardingSeen === true;
    ...
    attach(scope) {
        ...
        this.unsubscribe = scope.subscribe(() => {
            ...
            if (snapshot.status === 'ready' && snapshot.value && snapshot.value.onboardingSeen === this.optimisticSeen)
                this.optimisticSeen = undefined;
            this.emit();
        });
        this.emit();
    }
    ...
    markSeen() { this.write(true); }
    replay() { this.write(false); }
    write(next) {
        this.memorySeen = next;
        this.optimisticSeen = next;
        this.emit();
        const snapshot = this.scope?.getSnapshot();
        if (!this.scope || snapshot?.status !== 'ready' || !snapshot.writable)
            return;
        void this.scope.set('onboardingSeen', next).catch(() => {
            if (this.optimisticSeen === next)
                this.optimisticSeen = undefined;
            this.emit();
        });
    }
