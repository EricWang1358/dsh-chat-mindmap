...
export function cardStateOf(reference, url, error) {
    if (!reference)
        return { kind: 'failed', note: CARD_MISSING_NOTE };
    if (reference.state === 'expired')
        return { kind: 'expired', note: CARD_EXPIRED_NOTE };
    if (error)
        return { kind: 'failed', note: error };
    if (!url)
        return { kind: 'loading' };
    return { kind: 'ready' };
}
