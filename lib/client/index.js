...
function nodeCount(node) { let count = 0; const pending = [node]; while (pending.length) {
    const current = pending.pop();
    count += 1;
    pending.push(...(current.children ?? []));
} ; return count; }
function toRenderNode(node, depth, progressive) {
