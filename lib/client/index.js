...
function MapCanvas({ record, onDocumentChange, onXmind, onActions, onFullscreenChange }) {
    ...
    return createElement('div', { ref: fullscreenRef, style: { position: 'relative', flex: 1, minHeight: '480px', borderRadius: '8px', overflow: 'hidden', background: '#fff' } }, fullscreen ? createElement('button', { type: 'button', onClick: () => void window.document.exitFullscreen?.(), style: { position: 'absolute', top: '12px', right: '12px', zIndex: 2, ...buttonStyle() }, 'aria-label': '退出全屏画布' }, '退出全屏') : null, createElement('div', { ref: canvasRef, style: { width: '100%', height: '100%', minHeight: '480px' } }));
}
