...
export function validateAgentOutlineResult(value) {
    ...
    if (headingCount < 1)
        throw new DomainError('INVALID_AGENT_OUTLINE', 'agent outline needs a root heading');
    if (headingCount + bulletCount < 2)
        throw new DomainError('INVALID_AGENT_OUTLINE', 'agent outline needs at least one child heading or bullet');
