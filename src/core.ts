...
function limitsFrom(options?: MindmapBuildOptions): Required<MindmapBuildOptions> {
  return {
...
function makeNode(title: string, path: string, children?: MindmapNode[]): MindmapNode {
  return { id: nodeId(path), title: cleanText(title).slice(0, MAX_NODE_TITLE_LENGTH), ...(children?.length ? { children } : {}) }
}

type ParseLimits = Required<Pick<MindmapBuildOptions, 'maxNodes' | 'maxDepth' | 'maxChildren'>> & { nodeCount: number }

function parseMarkdownOutline(input: string, title: string, limits: ParseLimits): MindmapNode | null {
  const lines = input.split(/\r?\n/)
  const headingLines = lines.filter((line) => /^\s{0,3}#{1,6}\s+\S/.test(line))
  if (headingLines.length < 1) return null

  const rootHeading = headingLines[0]!.replace(/^\s*#+\s+/, '').trim() || title
  const root: MindmapNode = makeNode(rootHeading, 'root')
  limits.nodeCount = 1
  const stack: Array<{ level: number; node: MindmapNode }> = [{ level: 1, node: root }]
  const bulletStack: Array<MindmapNode | undefined> = []
  ...
  for (const line of lines) {
    ...
    if (match) {
      ...
      if (match[2]!.trim() === rootHeading && limits.nodeCount === 1) continue
      const level = match[1]!.length
      while (stack.length > 1 && stack[stack.length - 1]!.level >= level) stack.pop()
      const parent = stack[stack.length - 1]!.node
      const depth = stack.length
      if (depth > limits.maxDepth || limits.nodeCount >= limits.maxNodes) continue
      if ((parent.children?.length ?? 0) >= limits.maxChildren) continue
      ...
      if (stack.length < limits.maxDepth) stack.push({ level, node })
      continue
    }
    ...
    if (!bullet || limits.nodeCount >= limits.maxNodes) continue
    ...
    if (depth > limits.maxDepth || (parent.children?.length ?? 0) >= limits.maxChildren) continue
...
function parseTranscript(input: string, title: string, limits: ParseLimits): MindmapNode {
  ...
  for (const line of lines) {
    ...
    if (speaker) {
      ...
      if (content) speakerGroups.get(currentSpeaker)!.push(content)
    } else {
      speakerGroups.set(currentSpeaker, speakerGroups.get(currentSpeaker) ?? [])
  ...
  for (const [speaker, entries] of speakerGroups) {
    if (!entries.length || limits.nodeCount >= limits.maxNodes || (root.children?.length ?? 0) >= limits.maxChildren) continue
    const children: MindmapNode[] = []
    for (const [index, entry] of entries.slice(0, limits.maxChildren).entries()) {
      if (limits.nodeCount + 2 > limits.maxNodes) break
      children.push(makeNode(entry, `speaker-${groupIndex}-${index}`))
      limits.nodeCount += 1
    }
    if (!children.length) continue
    root.children ??= []
    if (limits.nodeCount + 1 > limits.maxNodes) break
  ...
  if (!root.children?.length) {
    ...
    for (const [index, line] of lines.slice(0, limits.maxChildren).entries()) {
      if (limits.nodeCount >= limits.maxNodes) break
...
export function buildMindmap(context: string, title = '', options?: MindmapBuildOptions): MindmapDocument {
...
export function mindmapToMarkdown(node: MindmapNode): string {
  const lines: string[] = []
  const pending: Array<{ node: MindmapNode; depth: number }> = [{ node, depth: 0 }]
  while (pending.length) {
    const current = pending.pop()!
 ...
 */
export function mindmapNodeNotesForPrompt(root: MindmapNode, maxCharacters = 96_000): { notes: MindmapNodeNoteReference[]; omitted: number } {
  const notes: MindmapNodeNoteReference[] = []
  const pending: Array<{ node: MindmapNode; path: string }> = [{ node: root, path: root.title }]
  let remaining = Math.max(0, Math.floor(maxCharacters))
  let omitted = 0
  while (pending.length) {
    const current = pending.pop()!
    const children = current.node.children ?? []
    for (let index = children.length - 1; index >= 0; index -= 1) pending.push({ node: children[index]!, path: `${current.path} > ${children[index]!.title}` })
    ...
    if (candidate.length > remaining) { omitted += 1; continue }
...
export function validateMindmapDocument(value: unknown, options?: MindmapValidationLimits): MindmapDocument {
  const limits = {
  ...
  if (document.version !== 1 || typeof document.title !== 'string' || document.title.length > limits.maxTitleLength) throw new Error('invalid document metadata')
  const source = document.source
  if (!source || typeof source !== 'object') throw new Error('document source is required')
  const sourceRecord = source as Record<string, unknown>
  if (sourceRecord.kind !== 'agent-context' || !Number.isInteger(sourceRecord.characters) || typeof sourceRecord.generatedAt !== 'string') throw new Error('invalid document source')

  const root = document.root
  if (!root || typeof root !== 'object') throw new Error('document root is required')
  let count = 0
  const pending: Array<{ value: unknown; depth: number }> = [{ value: root, depth: 0 }]
  while (pending.length) {
    const current = pending.pop()!
    if (!current.value || typeof current.value !== 'object') throw new Error('invalid mindmap node')
    const node = current.value as Record<string, unknown>
    if (typeof node.id !== 'string' || node.id.length === 0 || node.id.length > 200 || typeof node.title !== 'string' || node.title.length === 0 || node.title.length > 180) throw new Error('invalid mindmap node fields')
    if (typeof node.note !== 'undefined' && (typeof node.note !== 'string' || node.note.length > limits.maxNoteLength)) throw new Error('invalid mindmap node note')
    if (typeof node.collapsed !== 'undefined' && typeof node.collapsed !== 'boolean') throw new Error('invalid mindmap node collapsed state')
    if (current.depth > limits.maxDepth) throw new Error('mindmap depth exceeds limit')
    count += 1
    if (count > limits.maxNodes) throw new Error('mindmap node count exceeds limit')
    if (typeof node.children !== 'undefined') {
      if (!Array.isArray(node.children)) throw new Error('mindmap children must be an array')
...
export function flattenNode(node: MindmapNode): Array<{ title: string; depth: number }> {
