...
function parseMarkdownOutline(input: string, title: string): MindmapNode | null {
  ...
  for (const line of headingLines.slice(1)) {
    ...
    parent.children ??= []
    if (parent.children.length < MAX_CHILDREN) parent.children.push(node)
    if (stack.length < MAX_DEPTH) stack.push({ level, node })
  }
  return root.children?.length ? root : null
}

function parseTranscript(input: string, title: string): MindmapNode {
  ...
  for (const [speaker, entries] of speakerGroups) {
    ...
    const children = entries.slice(0, MAX_CHILDREN).map((entry, index) =>
      makeNode(entry, `speaker-${groupIndex}-${index}`),
    )
    root.children ??= []
    root.children.push(makeNode(speaker === 'user' ? '用户观点' : speaker === 'assistant' ? '助手观点' : speaker, `speaker-${groupIndex}`, children))
    groupIndex += 1
    if (root.children.length >= MAX_CHILDREN) break
  }
...
export function buildMindmap(context: string, title = ''): MindmapDocument {
