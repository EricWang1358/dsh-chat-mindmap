...
function panelStyle(): Record<string, string> { return { display: 'flex', flexDirection: 'column', height: '100%', minHeight: '0', overflow: 'hidden', padding: '0', background: 'var(--dsw-alias-bg-base,#f7f8fa)', color: 'var(--dsw-alias-label-primary,#202124)', font: '13px/1.45 system-ui,sans-serif' } }
...
function MapCanvas({ record, onDocumentChange, onXmind, onActions, onFullscreenChange, onNodeSelect }: { record: MindmapRecord; onDocumentChange: (document: MindmapDocument) => void; onXmind: (blob: Blob | null) => void; onActions: (actions: MapActions | null) => void; onFullscreenChange: (fullscreen: boolean) => void; onNodeSelect: (node: SelectedNode | null) => void }): ReactElement {
  ...
  return createElement('div', { ref: fullscreenRef, 'aria-busy': renderState === 'loading', style: { position: 'relative', width: '100%', minWidth: 0, minHeight: 0, flex: '1 1 0', borderRadius: '4px', overflow: 'hidden', background: shellDark ? '#24262c' : '#f9fafb' } },
    ...
    fullscreen ? createElement('div', { style: { position: 'absolute', top: '12px', left: '12px', right: '12px', zIndex: 4, display: 'flex', alignItems: 'center', gap: '8px', pointerEvents: 'none' } },
      createElement('span', { style: { padding: '5px 8px', borderRadius: '4px', background: 'rgba(15,23,42,.84)', color: '#e2e8f0', pointerEvents: 'auto' } }, '全屏编辑：双击节点直接改名，或使用右侧节点属性'),
...
function BrainmapView(props: ConvViewProps & { sessions: SessionService }): ReactElement {
  ...
  const createMap = () => {
    ...
    setStatus('正在生成未保存草稿…')
    void api<{ document: MindmapDocument }>('/generate', { method: 'POST', signal: controller.signal, headers: { 'content-type': 'application/json' }, body: JSON.stringify({ context: manualText, title, source: { kind: 'text', sessionId }, config: { instruction, maxNodes: draftMaxNodes }, save: false }) })
      .then(async (generated) => {
        ...
        setStatus('正在保存脑图…')
        await api<MindmapRecord>('/maps', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ title, document: generated.document, source: { kind: 'text', sessionId }, config: { instruction, maxNodes: draftMaxNodes } }) })
        setShowCreate(false); setManualText(''); void refresh(true); setStatus('已创建脑图')
      })
      .catch((error) => { if (controller.signal.aborted) setStatus('已取消创建，未保存任何内容'); else setStatus(`创建失败：${error instanceof Error ? error.message : String(error)}`) })
      .finally(() => { if (createControllerRef.current === controller) createControllerRef.current = null; setCreatePhase('idle') })
  }
  const regenerate = () => {
    if (!record) return
