...
function panelStyle(): Record<string, string> { return { display: 'flex', flexDirection: 'column', height: '100%', minHeight: '100%', overflow: 'hidden', padding: '0', background: 'var(--dsw-alias-bg-base,#f7f8fa)', color: 'var(--dsw-alias-label-primary,#202124)', font: '13px/1.45 system-ui,sans-serif' } }
  ...
  return createElement('main', { style: panelStyle() },
    ...
    createElement('div', { style: { display: 'grid', gridTemplateColumns: inspectorOpen ? '228px minmax(0,1fr) 300px' : '228px minmax(0,1fr)', position: 'relative', flex: 1, minHeight: 0, overflow: 'hidden', background: 'var(--dsw-alias-bg-layer-2,var(--dsw-alias-bg-base,#23262d))' } },
      ...
      record ? createElement('section', { style: { minWidth: 0, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 0 } },
        ...
        createElement('div', { style: { position: 'relative', flex: 1, minHeight: 0, overflow: 'hidden' } }, createElement(MapCanvas, { key: `${record.libraryId}:${record.current.source.generatedAt}`, record, onDocumentChange: persistDocument, onXmind: setXmind, onActions: setMapActions, onFullscreenChange: setCanvasFullscreen, onNodeSelect: selectNode }), createElement('div', { style: { position: 'absolute', right: '16px', bottom: '16px', zIndex: 2, display: 'flex', alignItems: 'center', gap: '2px', padding: '3px 5px', border: '1px solid color-mix(in srgb, var(--dsw-alias-border-l1,#2c3445) 78%, transparent)', borderRadius: '999px', background: 'color-mix(in srgb, var(--dsw-alias-bg-layer-1,#171e2e) 86%, transparent)', boxShadow: '0 4px 14px rgba(0,0,0,.12)', backdropFilter: 'blur(8px)' } }, createElement('button', { type: 'button', disabled: !mapActions, onClick: () => { mapActions?.zoomOut(); setZoom(mapActions?.zoomPercent() ?? zoom) }, style: zoomButtonStyle(), 'aria-label': '缩小画布' }, '−'), createElement('button', { type: 'button', disabled: !mapActions, onClick: () => { mapActions?.fit(); setZoom(mapActions?.zoomPercent() ?? 100) }, style: { ...zoomButtonStyle(), minWidth: '48px', fontSize: '12px' }, title: '适应画布', 'aria-label': '适应画布' }, `${zoom}%`), createElement('button', { type: 'button', disabled: !mapActions, onClick: () => { mapActions?.zoomIn(); setZoom(mapActions?.zoomPercent() ?? zoom) }, style: zoomButtonStyle(), 'aria-label': '放大画布' }, '＋'))))
    ...
    createElement('span', { role: 'status', style: { position: 'absolute', left: '244px', bottom: '8px', zIndex: 3, maxWidth: inspectorOpen ? 'calc(100% - 570px)' : 'calc(100% - 260px)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', padding: '2px 6px', color: 'var(--dsw-alias-label-secondary,#94a3b8)', fontSize: '11px', pointerEvents: 'none' } }, status),
  )
}
