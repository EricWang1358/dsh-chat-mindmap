...
type MindMapLike = {
  ...
  destroy?(): void
  view?: { enlarge(): void; narrow(): void; fit(): void; getTransformData?(): { transform?: { scale?: number } } }
}
...
function toRenderNode(node: MindmapNode, depth: number): MindmapNode {
  ...
  const collapseForInitialRender = depth >= DEFAULT_RENDER_COLLAPSE_DEPTH && hasChildren && node.collapsed !== false
...
function panelStyle(): Record<string, string> { return { display: 'flex', flexDirection: 'column', minHeight: '100%', padding: '0', background: 'var(--dsw-alias-bg-base,#f7f8fa)', color: 'var(--dsw-alias-label-primary,#202124)', font: '13px/1.45 system-ui,sans-serif' } }
function buttonStyle(): Record<string, string> { return { border: '1px solid var(--dsw-alias-border-l2,#e8eaed)', background: 'var(--dsw-alias-button-tool-bar-fill,#fff)', color: 'inherit', borderRadius: '4px', padding: '5px 8px', cursor: 'pointer' } }
function inputStyle(): Record<string, string> { return { display: 'block', width: '100%', boxSizing: 'border-box', padding: '7px', borderRadius: '4px', border: '1px solid #e8eaed', background: '#fff', color: 'inherit' } }
...
function MapCanvas({ record, onDocumentChange, onXmind, onActions, onFullscreenChange, onNodeSelect }: { record: MindmapRecord; onDocumentChange: (document: MindmapDocument) => void; onXmind: (blob: Blob | null) => void; onActions: (actions: MapActions | null) => void; onFullscreenChange: (fullscreen: boolean) => void; onNodeSelect: (node: SelectedNode | null) => void }): ReactElement {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      onActions({
        ...
        fit: () => instance.view?.fit(),
        zoomPercent: () => Math.round((instance.view?.getTransformData?.().transform?.scale ?? 1) * 100),
        saveNode: (node) => {
      ...
      const selected = (raw?: unknown) => {
        ...
        if (typeof id !== 'string' || typeof title !== 'string') { onNodeSelect(null); return }
        const note = node?.getData?.('note')
        onNodeSelect({ id, title, note: typeof note === 'string' ? note : '' })
      }
      const clearSelected = () => onNodeSelect(null)
...
function BrainmapView(props: ConvViewProps & { sessions: SessionService }): ReactElement {
  ...
  useEffect(() => { if (!selectedId) { setRecord(null); setInstruction(''); return }; void api<MindmapRecord>(`/maps/${encodeURIComponent(selectedId)}`).then((next) => { setRecord(next); setInstruction(next.config.instruction ?? '') }).catch((error) => setStatus(String(error))) }, [selectedId])
  useEffect(() => {
  ...
  const regenerate = () => {
    ...
    void api<PanelRunView>(`/maps/${encodeURIComponent(record.libraryId)}/regenerate`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sessionId, expectedUpdatedAt: record.updatedAt, ...(note ? { instruction: note } : {}) }) })
  ...
  return createElement('main', { style: panelStyle() },
    createElement('header', { style: { height: '48px', boxSizing: 'border-box', display: 'flex', alignItems: 'center', gap: '7px', padding: '0 14px', borderBottom: '1px solid #e8eaed', background: '#fff' } }, createElement('button', { type: 'button', style: buttonStyle(), 'aria-label': '展开脑图库' }, '☰'), createElement('strong', { style: { marginRight: 'auto', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' } }, record?.title ?? '脑图'), galleryState === 'loading' ? createElement('small', { role: 'status', style: { color: '#6b7280' } }, '读取图库…') : null, galleryState === 'failed' ? createElement('button', { type: 'button', onClick: () => void refresh(true), style: buttonStyle() }, '重试') : null, createElement('button', { type: 'button', disabled: !mapActions, onClick: () => void mapActions?.undo(), style: buttonStyle(), title: '撤销 Ctrl+Z', 'aria-label': '撤销 Ctrl+Z' }, '↶'), createElement('button', { type: 'button', disabled: !mapActions, onClick: () => void mapActions?.redo(), style: buttonStyle(), title: '重做 Ctrl+Shift+Z', 'aria-label': '重做 Ctrl+Shift+Z' }, '↷'), createElement('button', { type: 'button', onClick: () => setInspectorOpen((open) => !open), style: buttonStyle(), title: '节点属性', 'aria-label': '打开节点属性' }, '☷'), createElement('details', { style: { position: 'relative' } }, createElement('summary', { style: { ...buttonStyle(), listStyle: 'none', userSelect: 'none' }, 'aria-label': '更多操作' }, '···'), createElement('div', { style: { position: 'absolute', right: 0, top: 'calc(100% + 6px)', zIndex: 5, display: 'grid', gap: '5px', minWidth: '142px', padding: '7px', border: '1px solid #e8eaed', borderRadius: '4px', background: '#fff', boxShadow: '0 10px 24px rgba(0,0,0,.14)' } }, createElement('button', { type: 'button', disabled: !mapActions, onClick: () => void mapActions?.expandAll(), style: buttonStyle() }, '全部展开'), createElement('button', { type: 'button', disabled: !mapActions, onClick: () => void mapActions?.collapseAll(), style: buttonStyle() }, '全部折叠'), createElement('button', { type: 'button', disabled: !mapActions, onClick: () => void mapActions?.collapseToLevel(2), style: buttonStyle() }, '折叠至第 2 层'), createElement('button', { type: 'button', disabled: !mapActions, onClick: () => void mapActions?.openSvgPreview(), style: buttonStyle() }, '预览 SVG'), createElement('button', { type: 'button', disabled: panelRun?.status === 'running', onClick: regenerate, style: buttonStyle() }, '重新生成'))), createElement('button', { type: 'button', disabled: !mapActions, onClick: () => void mapActions?.toggleFullscreen(), style: buttonStyle(), title: '全屏画布', 'aria-label': '全屏画布' }, '⛶')),
