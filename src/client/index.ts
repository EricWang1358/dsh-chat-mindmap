...
function shellThemeConfig(theme: string, dark: boolean): ThemePreset {
...
function MapCanvas({ record, onDocumentChange, onXmind, onActions, onFullscreenChange, onNodeSelect }: { record: MindmapRecord; onDocumentChange: (document: MindmapDocument) => void; onXmind: (blob: Blob | null) => void; onActions: (actions: MapActions | null) => void; onFullscreenChange: (fullscreen: boolean) => void; onNodeSelect: (node: SelectedNode | null) => void }): ReactElement {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const instance = new MindMap({ el: canvasRef.current, data: toSimpleMindMapData(renderRoot), layout: recordRef.current.config.layout, theme: 'default', themeConfig: shellThemeConfig(recordRef.current.config.theme, shellDark).config, fit: true })
  ...
  return createElement('div', { ref: fullscreenRef, 'aria-busy': renderState === 'loading', style: { position: 'relative', flex: 1, minHeight: '480px', borderRadius: '4px', overflow: 'hidden', background: shellDark ? '#24262c' : '#f9fafb' } },
