...
type PanelRunView = { runId: string; libraryId: string; status: 'running' | 'completed' | 'failed' | 'cancelled'; detail: string; childId?: string; revisionId?: string }
  ...
  return createElement('main', { style: panelStyle() },
    ...
    createElement('div', { style: { display: 'grid', gridTemplateColumns: 'minmax(140px,220px) minmax(0,1fr)', gap: '12px', flex: 1, minHeight: 0 } },
      ...
      record ? createElement('section', { style: { minWidth: 0, display: 'flex', flexDirection: 'column', gap: '8px' } },
        ...
        panelRun?.libraryId === record.libraryId ? createElement('div', { role: 'status', style: { display: 'flex', alignItems: 'center', gap: '8px', padding: '7px 9px', borderRadius: '7px', background: panelRun.status === 'failed' ? 'rgba(127,29,29,.38)' : panelRun.status === 'completed' ? 'rgba(6,78,59,.38)' : 'rgba(30,41,59,.72)', border: '1px solid #475569' } }, createElement('strong', null, panelRun.status === 'running' ? 'Fork 子代理运行中' : panelRun.status === 'completed' ? '重新生成完成' : panelRun.status === 'cancelled' ? '重新生成已取消' : '重新生成失败'), createElement('span', { style: { opacity: .78 } }, panelRun.detail), panelRun.childId ? createElement('code', { style: { opacity: .62, fontSize: '11px' } }, `子代理 ${panelRun.childId}`) : null, panelRun.status === 'running' ? createElement('button', { type: 'button', onClick: cancelRegenerate, style: { ...buttonStyle(), marginLeft: 'auto' } }, '取消') : null) : null,
