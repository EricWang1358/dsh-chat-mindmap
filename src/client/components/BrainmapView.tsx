...
export function EmptyState({ kind }: { kind: EmptyKind }): ReactElement {
  const copy = kind === 'capability'
    ? ['脑图能力不可用', '脑图服务未就绪或当前部署缺少所需能力，请稍后重试。']
    : kind === 'workspace'
      ? ['当前工作区暂无脑图', '切回“本会话”查看当前会话脑图，或点击“＋”新建。']
      : ['当前会话暂无脑图', '点击“＋”新建，或在聊天里让 Agent 从文本/PDF/附件生成。']
  return createElement('section', { style: { display: 'grid', placeItems: 'center', minHeight: '480px', gap: '8px', opacity: .75 } },
    createElement('strong', null, copy[0]),
    createElement('small', null, copy[1]))
}
...
function RegenerateModal({ record, panelRunning, sessionAvailable, draft, onDraftChange, onClose, onConfirm }: RegenerateModalProps): ReactElement {
  ...
  return createElement('div', { role: 'dialog', 'aria-modal': true, 'aria-label': '重新生成脑图', onClick: onClose, style: { position: 'fixed', inset: 0, zIndex: 30, display: 'grid', placeItems: 'center', background: 'rgba(15,23,42,.55)' } },
    createElement('form', { onSubmit: (event: Event) => { event.preventDefault(); if (!panelRunning && sessionAvailable) onConfirm() }, onClick: (event: Event) => event.stopPropagation(), style: { display: 'grid', gap: '10px', width: 'min(440px, calc(100vw - 48px))', padding: '16px', border: '1px solid var(--dsw-alias-border-l1,#2c3445)', borderRadius: '10px', background: 'var(--dsw-alias-bg-layer-1,var(--dsw-alias-bg-base,#171e2e))' } },
      ...
      createElement('div', { style: { display: 'flex', gap: '8px', justifyContent: 'flex-end' } },
        createElement('button', { type: 'button', onClick: onClose, style: buttonStyle() }, '取消'),
        createElement('button', { type: 'submit', disabled: panelRunning || !sessionAvailable, 'data-primary-regenerate': 'true', style: { ...buttonStyle(), borderColor: 'var(--dsw-alias-brand-primary,#14b8a6)' } }, panelRunning ? '生成中…' : '确认生成'))))
}
...
function BrainmapView(props: ConvViewProps & { sessions: SessionService }): ReactElement {
  ...
  const persistDocument = (document: MindmapDocument) => {
    ...
    if (!current) return
    void api<MindmapRecord>(`/maps/${encodeURIComponent(current.libraryId)}`, { method: 'PATCH', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ document, rotatePrevious: false, expectedRecordVersion: current.recordVersion }) }).then((next) => { setRecord((prev) => prev ? { ...prev, updatedAt: next.updatedAt, current: next.current, previous: next.previous, recordVersion: next.recordVersion } : next); setStatus('已自动保存当前手动修改') }).catch((error) => {
  ...
  return createElement('main', { style: panelStyle() },
    ...
    createElement('div', { ref: workspaceRef, style: { display: 'grid', gridTemplateColumns: sidebarOpen ? (workspaceWidth > 0 && workspaceWidth < 900 ? '56px minmax(0,1fr)' : '228px minmax(0,1fr)') : '0px minmax(0,1fr)', position: 'relative', flex: '1 1 0', minWidth: 0, minHeight: 0, overflow: 'hidden', background: 'var(--dsw-alias-bg-layer-2,var(--dsw-alias-bg-base,#23262d))' } },
      ...
      createElement('div', { style: { display: 'flex', flexDirection: 'column', position: 'relative', flex: 1, minWidth: 0, minHeight: 0, overflow: 'hidden' } }, createElement(MapCanvas, { key: `${record.libraryId}:${record.current.source.generatedAt}`, record, onDocumentChange: persistDocument, onXmind: setXmind, onActions: setMapActions, onFullscreenChange: () => undefined, onNodeSelect: selectNode }), createElement('div', { style: { position: 'absolute', right: '16px', bottom: '16px', zIndex: 2, display: 'flex', alignItems: 'center', gap: '2px', padding: '3px 5px', border: '1px solid color-mix(in srgb, var(--dsw-alias-border-l1,#2c3445) 78%, transparent)', borderRadius: '999px', background: 'color-mix(in srgb, var(--dsw-alias-bg-layer-1,#171e2e) 86%, transparent)', boxShadow: '0 4px 14px rgba(0,0,0,.12)', backdropFilter: 'blur(8px)' } }, createElement('button', { type: 'button', disabled: !mapActions, onClick: () => mapActions?.zoomOut(), style: zoomButtonStyle(), 'aria-label': '缩小画布' }, '−'), createElement('button', { type: 'button', disabled: !mapActions, onClick: () => mapActions?.zoomIn(), style: zoomButtonStyle(), 'aria-label': '放大画布' }, '＋'))))
        : createElement(EmptyState, { kind: galleryState === 'failed' ? 'capability' : scope === 'workspace' ? 'workspace' : 'session' })),
