...
export interface BlobUrlLruStats { created: number; revoked: number; evicted: number }
...
export function createBlobUrlLru(options: { capacity: number; create(blob: Blob): string; revoke(url: string): void }): BlobUrlLru {
  const entries = new Map<string, string>()
  const stats = { created: 0, revoked: 0, evicted: 0 }
  const revokeQuietly = (url: string): void => {
    try {
      options.revoke(url)
      stats.revoked += 1
    } catch {
  ...
  return {
    put(key, blob) {
      ...
      stats.created += 1
      entries.set(key, url)
      while (entries.size > Math.max(1, options.capacity)) dropOldest()
      return url
    },
    get(key) {
    ...
    disposeAll() {
      for (const url of entries.values()) revokeQuietly(url)
      entries.clear()
    },
    stats() {
      return { ...stats }
...
let singleton: BlobUrlLru | null = null

export function getBlobUrlLru(): BlobUrlLru {
  if (!singleton) {
