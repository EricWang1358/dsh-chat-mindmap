...
## D-S4-3（W2）：WBS 编号映射与视图文件拆分范围

- **编号映射**：本计划自设 W1＝host 集成切换（源自 W0(b) 裁决与 S3 移交第 1 条，用户任务表未单列）；本计划 W2 ≈ 用户表 S4-W1（信息架构）并**吸收用户表 S4-W2（常驻与收纳）的菜单/Modal/负断言项**；后续 W3–W7 与用户表对齐。提交引用同时标注两套编号。
- **拆分范围**：MapCanvas 与 MapList 暂不独立成文件（§5 为建议结构）——字节级搬移优先保证行为零回归；W3 primitives 重写本就要重写该标记，届时一并拆出。api.ts/artifact-html.ts 已独立。

## D-S4-4（W2，过程记录）：长行文件的读改写管线教训

- **事实**：tools.read 对超过 2000 字符的单行仅返回截断显示；以「read→join→patch→write」整文件往返会静默丢失超长行尾部（本次 3801 字符的 header 行被截断，菜单段消失）。git checkout + 切分脚本恢复后，改造改走「node 脚本批量补丁＋tools.edit 单点编辑（锚点强制唯一）」，typecheck/test 全绿验证无损。
