...
// The former inline ellipsis menu is now a mutually exclusive utility dock.
assert.match(viewSource, /function MorePanel/)
assert.match(viewSource, /更多脑图操作/)
...
assert.match(viewSource, /color-mix\(in srgb, var\(--dsw-alias-brand-primary/)
...
assert.match(viewSource, /正在启动 fork 子代理/)
...
assert.match(idxSource, /MINDMAP_VIEW_ID/)
